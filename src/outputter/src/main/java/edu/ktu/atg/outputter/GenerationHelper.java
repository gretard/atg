...
public final class GenerationHelper {
	...
	protected static String generateNameIfArray(final IExecutable item) {
		...
		if (item.getClassz() != null && item.getClassz().isArray()) {
			long c = pName.chars().filter(ch -> ch == '[').count();
			Class<?> component = item.getClassz();
			while (component.getComponentType() != null) {
				component = component.getComponentType();
			}
			pName = component.getName().replace('$', '.') + StringUtils.repeat("[]", (int) c);
		}
